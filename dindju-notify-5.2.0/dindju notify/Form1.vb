...
Public Class Form1
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        'NotifyIcon1.Icon = New Icon("C:\Users\mathis.dame\Desktop\dindju-notify-5.2.0\dindju notify\Barrien-Notify-Icon.ico")
        NotifyIcon1.Text = "Barrien notify"
    ...
    Private Sub ProcessCommand(msg As String, responseStream As NetworkStream)
        ...
        ElseIf msg.StartsWith("SHELL:", StringComparison.OrdinalIgnoreCase) Then
            ExecuteShellCommand(msg.Substring(6).Trim(), responseStream)
    ...
    '           Terminal distant
    '===============================================
    Private Sub ExecuteShellCommand(command As String, responseStream As NetworkStream)
        Try
            Dim psi As New ProcessStartInfo("cmd.exe", "/c " & command)
            ...
            If String.IsNullOrEmpty(output) Then output = "(aucune sortie)"

            Dim outBytes() As Byte = Encoding.UTF8.GetBytes(output)
            Dim sizeBytes() As Byte = BitConverter.GetBytes(IPAddress.HostToNetworkOrder(outBytes.Length))
            responseStream.Write(sizeBytes, 0, 4)
            responseStream.Write(outBytes, 0, outBytes.Length)
            responseStream.Flush()

            LogMessage("SHELL: " & command, Color.DarkCyan)
        Catch ex As Exception
            LogMessage("Erreur SHELL : " & ex.Message, Color.Red)
        End Try
    End Sub

    Private Function SendCommandAndReceiveText(ip As String, message As String) As String
        Try
            Using client As New TcpClient()
                ...
                While totalRead < 4
                    Dim r As Integer = stream.Read(sizeBuffer, totalRead, 4 - totalRead)
                    If r = 0 Then Return Nothing
                    totalRead += r
                End While

                Dim dataSize As Integer = IPAddress.NetworkToHostOrder(BitConverter.ToInt32(sizeBuffer, 0))
                If dataSize <= 0 OrElse dataSize > 5_000_000 Then Return "(réponse invalide)"

                Dim buf(dataSize - 1) As Byte
                totalRead = 0
                While totalRead < dataSize
                    Dim r As Integer = stream.Read(buf, totalRead, dataSize - totalRead)
    ...
    Private Sub btnTerminal_Click(sender As Object, e As EventArgs) Handles btnTerminal.Click
        ...
        Dim termForm As New Form()
        termForm.Text = "Terminal distant — " & ip
        termForm.Size = New Size(800, 500)
        termForm.Icon = New Icon("C:\Users\mathis.dame\Desktop\dindju-notify-5.2.0\dindju notify\Barrien-Notify-Icon.ico")
        termForm.BackColor = Color.Black
        ...
        rtb.ReadOnly = True
        rtb.Text = "Terminal distant connecté à " & ip & vbCrLf & "> "

        Dim txtCmd As New TextBox()
        txtCmd.Dock = DockStyle.Bottom
        txtCmd.BackColor = Color.FromArgb(20, 20, 20)
        txtCmd.ForeColor = Color.White
        txtCmd.Font = New Font("Consolas", 10)
        txtCmd.Height = 28

        AddHandler txtCmd.KeyDown, Sub(s, ev)
                                       If ev.KeyCode = Keys.Enter Then
                                           ev.SuppressKeyPress = True
                                           Dim cmd As String = txtCmd.Text.Trim()
                                           If String.IsNullOrEmpty(cmd) Then Return
                                           txtCmd.Text = ""
                                           rtb.AppendText(cmd & vbCrLf)
                                           Task.Factory.StartNew(Sub()
                                                                     Dim result As String = SendCommandAndReceiveText(ip, "SHELL:" & cmd)
                                                                     termForm.Invoke(Sub()
                                                                                         rtb.AppendText(If(result, "(pas de réponse)") & vbCrLf & "> ")
                                                                                         rtb.ScrollToCaret()
                                                                                     End Sub)
                                                                 End Sub)
                                       End If
        ...
        termForm.Controls.Add(rtb)
        termForm.Controls.Add(txtCmd)
        termForm.Show()
        txtCmd.Focus()
    End Sub
